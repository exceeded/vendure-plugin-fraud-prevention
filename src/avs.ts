 ...
 *  Missing values never count as a mismatch. */
 ...
 *
 * `expand[]=latest_charge` needs API version 2022-11-15 or newer; on an
 * account pinned earlier `latest_charge` is absent and `charges.data`
 * is present instead. When neither carries the charge object, the
...
export async function fetchStripeAvs(
    ...
    if (!charge && Array.isArray(pi.charges?.data) && pi.charges.data.length) {
        charge = pi.charges.data[pi.charges.data.length - 1];
    }
    if (!charge) {
